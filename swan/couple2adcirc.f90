...
!Casey 110422: Create new data structure with discontinuous information for SWAN.
      REAL, ALLOCATABLE :: DG_JDP1(:)
      REAL, ALLOCATABLE :: DG_JDP2(:)
      REAL, ALLOCATABLE :: DG_JVX2(:)
      REAL, ALLOCATABLE :: DG_JVY2(:)
      REAL, ALLOCATABLE :: DG_JWLV2(:)
      TYPE DATA2PASS
         INTEGER              :: NO_NBORS
         INTEGER,ALLOCATABLE  :: NBOR_EL(:)
         REAL,ALLOCATABLE :: ETA1(:)
         REAL,ALLOCATABLE :: ETA2(:)
         REAL,ALLOCATABLE :: UU2(:)
         REAL,ALLOCATABLE :: VV2(:)
         REAL,ALLOCATABLE :: VELMAG(:)
      ENDTYPE
      ...
      SUBROUTINE GrabDiscontinuousInformation(icell)

      USE M_GENARR,     ONLY: DEPTH
      USE SwanCompdata, ONLY: vs
      USE SwanGriddata, ONLY: nverts
      USE SWCOMM3,      ONLY: JDP1,JDP2,JVX2,JVY2,JWLV2
      ...
      IF(.NOT.ALLOCATED(DG_JDP1))  ALLOCATE(DG_JDP1(1:nverts))
      IF(.NOT.ALLOCATED(DG_JDP2))  ALLOCATE(DG_JDP2(1:nverts))
      IF(.NOT.ALLOCATED(DG_JVX2))  ALLOCATE(DG_JVX2(1:nverts))
      IF(.NOT.ALLOCATED(DG_JVY2))  ALLOCATE(DG_JVY2(1:nverts))
      IF(.NOT.ALLOCATED(DG_JWLV2)) ALLOCATE(DG_JWLV2(1:nverts))
